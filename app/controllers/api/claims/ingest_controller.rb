...
skip_before_action :authenticate_user!, only: %i[upload upload_fix runs_index steps_index steps_by_session_index run_ocr run_genai run_show run_invoices_index steps_by_invoice_index admin_submit_batch]
skip_before_action :require_confirmation, only: %i[upload upload_fix runs_index steps_index steps_by_session_index run_ocr run_genai run_show run_invoices_index steps_by_invoice_index admin_submit_batch]
skip_after_action  :verify_authorized,   only: %i[upload upload_fix runs_index steps_index steps_by_session_index run_ocr run_genai run_show run_invoices_index steps_by_invoice_index admin_submit_batch]
skip_forgery_protection only: %i[upload upload_fix runs_index steps_index steps_by_session_index run_ocr run_genai run_show run_invoices_index steps_by_invoice_index admin_submit_batch]
...
def run_ocr
  ...
  jid = ::Claims::RunOcrJob.perform_async(invoice_version_id, ingest_run_id, validationgenai_ruleset_id)
...
def run_show
  ...
  Rails.logger.error("[claims][ingest][run_show] ERROR: #{e.class}: #{e.message}")
...
def run_invoices_index
  ...
  versions = ::Claims::InvoiceVersion
    .joins(:invoice)
    .where(id: invoice_version_ids)
    .order(created_at: :asc)

  rows = versions.map do |iv|
    inv = iv.invoice
    {
      invoice_id: inv.id,
  ...
  Rails.logger.error("[claims][ingest][run_invoices_index] ERROR: #{e.class}: #{e.message}")
  render json: { rows: [], error: e.message }, status: :unprocessable_entity
end
...
def steps_by_invoice_index
  ...
  scope = ::Claims::IngestStepRun
    .joins("JOIN claims.invoice_versions iv ON iv.id = claims.ingest_step_runs.invoice_version_id")
    .where("iv.invoice_id = ?", invoice_id)

  scope = scope.where(ingest_run_id: ingest_run_id) if ingest_run_id

  rows = scope.order(created_at: :desc).limit(limit).map do |r|
  ...
  Rails.logger.error("[claims][ingest][steps_by_invoice_index] ERROR: #{e.class}: #{e.message}")
  render json: { rows: [], error: e.message }, status: :unprocessable_entity
end
...
def admin_submit_batch
  ...
  raise "Missing validationgenai_ruleset_id" if validationgenai_ruleset_id.empty?
  ...
  session_result = ::Claims::Sessions::Create.call(contractor_id: contractor_id)
  session_id = session_result.session.id
  created_session = true

  now = Time.current
  ingest_run = ::Claims::IngestRun.create!(
    session_id: session_id,
    status: "queued",
  ...
  files.each_with_index do |f, idx|
    ...
    begin
      ...
      node_resp = ingest_node_upload_pdf!(
        session_id: session_id,
      ...
      raise "Node upload returned no storage_key" if final_storage_key.to_s.strip.empty?
      ...
      jid = ::Claims::RunOcrJob.perform_async(
        invoice_version.id,
        ingest_run.id,
        validationgenai_ruleset_id
      )
      ...
      begin
        if invoice
          failed_status = invoice.status.to_s.start_with?("upload_") ? "upload_failed" : "ocr_failed"
  ...
  Rails.logger.error("[claims][ingest][admin_submit_batch] ERROR: #{e.class}: #{e.message}")
  render json: { ok: false, error: e.message }, status: :unprocessable_entity
end
...
def ingest_node_upload_pdf!(session_id:, invoice_version_id:, file:)
  ...
  filename = file_safe_call(file, :original_filename) || File.basename(file.path)

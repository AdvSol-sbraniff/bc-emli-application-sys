    ...
    class UserEligibilitycodesAdminController < ApplicationController
      ...
      def index
        ...
        sort =
          params[:sort].to_s.strip.presence ||
            "users_eligibilitycode_updated_at:desc"
        ...
        rows =
          scope.offset((page - 1) * per).limit(per).map { |r| serialize_row(r) }
      ...
      def order_clause(sort)
        ...
        column =
          case key
          when "email"
            "claims.v_user_eligibilitycodes.email"
          when "role"
            "claims.v_user_eligibilitycodes.role"
          when "first_name"
            "claims.v_user_eligibilitycodes.first_name"
          when "last_name"
            "claims.v_user_eligibilitycodes.last_name"
          ...
          when "approved_at"
            "claims.v_user_eligibilitycodes.approved_at"
          when "expires_at"
            "claims.v_user_eligibilitycodes.expires_at"
          when "users_eligibilitycode_created_at"
            "claims.v_user_eligibilitycodes.users_eligibilitycode_created_at"
          when "users_eligibilitycode_updated_at"

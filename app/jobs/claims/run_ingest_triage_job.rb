...
module Claims
  class RunIngestTriageJob
    ...
    def perform(ingest_document_id, ingest_run_id = nil, validationgenai_ruleset_id = nil)
      ...
      raise "Missing ingest_documents.di_read_raw_json for ingest_document_id=#{document.id}" if document.di_read_raw_json.blank?
      ...
      contextwindowjson =
        build_classifier_contextwindowjson(di_raw_json: document.di_read_raw_json)
      ...
      raise "ApplyDocumentTriageResult failed: #{result.inspect}" unless result[:ok]
    ...
    def build_classifier_contextwindowjson(di_raw_json:)
      ...
      raise "validationgenai_config.classifier_system_record is empty" if sys.strip.empty?
      ...
      messages << {
        ...
        content: [
          {
            type: "input_text",
            text: "Document Intelligence raw json:\n#{di_raw_json.to_json}"
          }
        ]
      }

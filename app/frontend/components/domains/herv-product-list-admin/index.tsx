...
type ProductRow = {
  ...
  sensible_heat_recovery_efficiency_sre_at_0c?: string | number | null;
  sensible_heat_recovery_efficiency_sre_at_minus_25c?: string | number | null;
  associated_net_supply_airflow_at_0c_cfm?: string | number | null;
  associated_net_supply_airflow_at_minus_25c_cfm?: string | number | null;
  associated_power_consumption_at_0c_w?: string | number | null;
  associated_power_consumption_at_minus_25c_w?: string | number | null;
  max_rated_airflow_at_0c_cfm?: string | number | null;
  power_consumption_at_0c_w?: string | number | null;
...
export default function HervProductListAdminScreen() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/claims/admin/herv_products/import_status', {
        method: 'GET',
      ...
      setError(e?.message || 'Failed to load HERV import status.');
    } finally {
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/claims/admin/herv_products?${params.toString()}`, {
        method: 'GET',
        headers: { Accept: 'application/json' },
        credentials: 'include',
      });
      ...
      setRows([]);
      setTotal(0);
      setError(e?.message || 'Failed to load HERV products.');
    } finally {
  ...
  const importSource = async (source: SourceStatus) => {
    ...
    try {
      const res = await fetch('/api/claims/admin/herv_products/import_downloaded_csv', {
        method: 'POST',
      ...
      setError(e?.message || 'HERV import failed.');
      await fetchStatus();
    } finally {
  ...
  return (
    <Flex as="main" direction="column" w="full" bg="greys.white" pb="24" minH="100vh">
      <ThinBlueTitleBar title="HERV ENERGY STAR Product List Config" />
          ...
          <Flex justify="space-between" gap={4} wrap="wrap" align="start">
            <Box>
              <Text fontSize="lg" fontWeight="bold">
                NRCan ENERGY STAR heat/energy recovery ventilator product list
              </Text>
              <Text fontSize="sm" opacity={0.75} mt={1} maxW="840px">
                This cached NRCan ENERGY STAR HRV/ERV list is used by the ventilation code rule for HERV product-list
                matching.
              </Text>
            </Box>
                  ...
                  <Tr>
                    <Td colSpan={5}>
                      <Flex align="center" gap={2} py={3}>
                        <Spinner size="sm" />
                        <Text>Loading HERV source list...</Text>
                      </Flex>
                    </Td>
                  </Tr>
                ) : (
          ...
          <Flex gap={3} align="end" wrap="wrap" mb={4}>
            <Box flex="1" minW="300px">
              <Text fontSize="xs" opacity={0.7} mb={1}>
                Search brand, model, or model type
              </Text>
              <Input
                value={q}
                onChange={(e) => {
                ...
                placeholder="Example: Broan or V160E75RT"
                bg="white"
              />
            </Box>
            ...
            <Table size="sm" minW="1300px">
              <Thead bg="gray.50">
                <Tr>
                  ...
                  <Th>Model</Th>
                  <Th>Type</Th>
                  <Th isNumeric>SRE 0C</Th>
                  <Th isNumeric>SRE -25C</Th>
                  <Th isNumeric>Airflow 0C CFM</Th>
                  <Th isNumeric>Airflow -25C CFM</Th>
                  <Th isNumeric>Power 0C W</Th>
                  <Th isNumeric>Power -25C W</Th>
                  <Th isNumeric>Max airflow 0C CFM</Th>
                  <Th>Source</Th>
                </Tr>
              </Thead>
              <Tbody>
                {loadingRows ? (
                  ...
                  <Tr>
                    <Td colSpan={11}>
                      <Text py={3} opacity={0.8}>
                        No HERV product rows found. Import the NRCan CSV first, or adjust your search.
                      </Text>
                    </Td>
                  </Tr>
                ) : (
                  rows.map((row) => (
                    <Tr key={row.id}>
                      ...
                      <Td fontWeight="semibold">{fmtValue(row.model_number)}</Td>
                      <Td>{fmtValue(row.model_type)}</Td>
                      <Td isNumeric>{fmtValue(row.sensible_heat_recovery_efficiency_sre_at_0c)}</Td>
                      <Td isNumeric>{fmtValue(row.sensible_heat_recovery_efficiency_sre_at_minus_25c)}</Td>
                      <Td isNumeric>{fmtValue(row.associated_net_supply_airflow_at_0c_cfm)}</Td>
                      <Td isNumeric>{fmtValue(row.associated_net_supply_airflow_at_minus_25c_cfm)}</Td>
                      <Td isNumeric>{fmtValue(row.associated_power_consumption_at_0c_w)}</Td>
                      <Td isNumeric>{fmtValue(row.associated_power_consumption_at_minus_25c_w)}</Td>
                      <Td isNumeric>{fmtValue(row.max_rated_airflow_at_0c_cfm)}</Td>

...
export default function ContractorFixUploadScreen() {
  ...
  const invoiceRowsInProposal = useMemo(() => proposedRows.filter((row) => row.fileRole === 'invoice'), [proposedRows]);
  ...
  const validationMessage = useMemo(() => {
    ...
    if (invoiceRowsInProposal.length === 0) return 'The proposed package must keep or add one invoice file.';
    if (invoiceRowsInProposal.length > 1) return 'The proposed package has more than one invoice file selected.';
    return '';
  }, [currentVersionRows, fixAvailabilityMessage, invoiceRowsInProposal.length, newRows.length, proposedRows]);
  ...
  const loadRunInvoices = useCallback(async (id: string) => {
    ...
    try {
      ...
      setRowsError(error?.message || 'Failed to load the updated invoice status.');
      setInvoiceRows([]);
    }
  }, []);
  ...
  const processingStoryLabel = (() => {
    ...
    if (!invoiceRows.length) return 'Reading revised files';

    const statuses = invoiceRows.map((row) => String(row.invoice_status || '').toLowerCase());
    if (statuses.some((status) => status.startsWith('upload_'))) return 'Uploading revised package';
    if (statuses.some((status) => status.startsWith('ocr_'))) return 'Reading revised files';
    if (statuses.some((status) => status === 'ocr_complete')) return 'Extracting revised invoice evidence';
    if (statuses.some((status) => status.startsWith('genai_'))) return 'Refreshing AI Advice';
  ...
  const toggleNewFileRole = (id: string) => {
    if (controlsLocked) return;
    setProposedRows((existing) =>
      existing.map((row) =>
        row.id === id && row.source === 'new'
          ? {
              ...row,
          ...
          <Box bg="white" p={5} borderRadius="xl" boxShadow="0 12px 34px rgba(15, 23, 42, 0.06)">
            ...
            <Flex justify="space-between" align="center" gap={3} wrap="wrap" mb={3}>
              <Box>
                ...
                <Text fontSize="xs" opacity={0.68}>
                  New files are staged as NEW ADDITION. The AI pipeline will classify the final package.
                </Text>
              </Box>
              <Button
                    ...
                    <Td>
                      {row.source === 'new' ? (
                        <Button
                          size="xs"
                          variant="ghost"
                          onClick={() => toggleNewFileRole(row.id)}

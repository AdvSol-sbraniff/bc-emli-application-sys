...
type ProductRow = {
  ...
  airflow_1_cfm?: string | number | null;
  efficacy_1_cfm_watt?: string | number | null;
  sound_level_sones?: string | number | null;
  bathroom_utility_airflow_at_0_25_in_wg?: string | number | null;
  markets?: string | null;
  energy_star_unique_id?: string | null;
  cb_model_identifier?: string | null;
...
export default function VentFanProductListAdminScreen() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/claims/admin/vent_fan_products/import_status', {
        method: 'GET',
      ...
      setError(e?.message || 'Failed to load ENERGY STAR fan import status.');
    } finally {
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/claims/admin/vent_fan_products?${params.toString()}`, {
        method: 'GET',
        headers: { Accept: 'application/json' },
        credentials: 'include',
      });
      ...
      setRows([]);
      setTotal(0);
      setError(e?.message || 'Failed to load ENERGY STAR fan products.');
    } finally {
  ...
  const importSource = async (source: SourceStatus) => {
    ...
    try {
      const res = await fetch('/api/claims/admin/vent_fan_products/import_downloaded_csv', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
          vent_fan_source_id: source.id,
          publishing_notes: 'ENERGY STAR certified ventilating fans CSV import',
        }),
      });
      const data = await res.json().catch(() => ({}));
      ...
      setError(e?.message || 'ENERGY STAR fan import failed.');
      await fetchStatus();
    } finally {
  ...
  return (
    <Flex as="main" direction="column" w="full" bg="greys.white" pb="24" minH="100vh">
      <ThinBlueTitleBar title="ENERGY STAR Ventilating Fan Product List Config" />
          ...
          <Flex justify="space-between" gap={4} wrap="wrap" align="start">
            <Box>
              <Text fontSize="lg" fontWeight="bold">
                ENERGY STAR certified ventilating fan product list
              </Text>
              <Text fontSize="sm" opacity={0.75} mt={1} maxW="840px">
                This cached EPA/DOE ENERGY STAR fan list is used by the ventilation code rule for bathroom, utility, and
                exhaust fan product-list matching.
              </Text>
            </Box>
                  ...
                  <Tr>
                    <Td colSpan={5}>
                      <Flex align="center" gap={2} py={3}>
                        <Spinner size="sm" />
                        <Text>Loading fan source list...</Text>
                      </Flex>
                    </Td>
                  </Tr>
                ) : (
          ...
          <Flex gap={3} align="end" wrap="wrap" mb={4}>
            <Box flex="1" minW="300px">
              <Text fontSize="xs" opacity={0.7} mb={1}>
                Search brand, model, type, market, or ENERGY STAR ID
              </Text>
              <Input
                value={q}
                onChange={(e) => {
                ...
                placeholder="Example: ACIQ or AEP110"
                bg="white"
              />
            </Box>
            ...
            <Table size="sm" minW="1400px">
              <Thead bg="gray.50">
                <Tr>
                  ...
                  <Th>Model name</Th>
                  <Th>Type</Th>
                  <Th isNumeric>Airflow 1 CFM</Th>
                  <Th isNumeric>Efficacy 1</Th>
                  <Th isNumeric>Sones</Th>
                  <Th isNumeric>0.25 in WG airflow</Th>
                  <Th>Markets</Th>
                  <Th>ENERGY STAR ID</Th>
                ...
                {loadingRows ? (
                  ...
                  <Tr>
                    <Td colSpan={12}>
                      <Text py={3} opacity={0.8}>
                        No ENERGY STAR fan product rows found. Import the CSV first, or adjust your search.
                      </Text>
                    </Td>
                  </Tr>
                ) : (
                  rows.map((row) => (
                    <Tr key={row.id}>
                      ...
                      <Td>{fmtValue(row.product_model_name)}</Td>
                      <Td>{fmtValue(row.fan_type)}</Td>
                      <Td isNumeric>{fmtValue(row.airflow_1_cfm)}</Td>
                      <Td isNumeric>{fmtValue(row.efficacy_1_cfm_watt)}</Td>
                      <Td isNumeric>{fmtValue(row.sound_level_sones)}</Td>
                      <Td isNumeric>{fmtValue(row.bathroom_utility_airflow_at_0_25_in_wg)}</Td>
                      <Td>{fmtValue(row.markets)}</Td>
                      <Td>{fmtValue(row.energy_star_unique_id)}</Td>
                      <Td>{fmtValue(row.cb_model_identifier)}</Td>

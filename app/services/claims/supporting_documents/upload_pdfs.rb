...
module Claims
  module SupportingDocuments
    class UploadPdfs
      Result =
        Struct.new(:uploaded_count, :rows) do
          def to_h
            { ok: true, uploaded_count: uploaded_count, rows: rows }
          end
        end
      ...
      def create_supporting_document!(invoice:, session_id:, file:)
        filename =
          safe_call(file, :original_filename).presence ||
      ...
      def serialize_supporting_document(document)
...

        {
          ...
          supplement_routing_quality: document.supplement_routing_quality,
          supplement_routing_quality_reason:
            document.supplement_routing_quality_reason,
          located_fields: serialize_located_fields(document),
      ...
      def serialize_located_fields(document)
        document
      ...
      def node_upload_supporting_pdf!(
        session_id:,
        invoice_id:,
        supporting_document_id:,
        file:

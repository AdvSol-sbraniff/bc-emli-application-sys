      ...
      def call
        ...
        representative_invoice =
          ::Claims::Invoice
            ...
            .order(:created_at, :id)
            .first
        if representative_invoice.nil?
          raise "Cannot infer contractor_id for session_id=#{@session_id}. Upload a batch through contractor context first."
        end
        ...
        Rails.logger.info(
          "[claims][ingest][#{stage}] run_id=#{run_id} ingest_run_id=nil session_id=#{@session_id} files=#{@files.size}"
        )
        ...
        Rails.logger.info(
          "[claims][ingest][#{stage}] run_id=#{run_id} ingest_run_id=nil file1=#{name} type=#{ctype} size=#{size}"
        )
        ...
        begin
          ...
          Rails.logger.error(
            "[claims][ingest][#{stage}] run_id=#{run_id} ingest_run_id=nil file1 FAILED: #{e.class}: #{e.message}"
          )

  ...
  module CodeRules
    module HealthSafety
      class ApplyRebateCap
        UPGRADE_TYPE_KEY = "health_and_safety_remediation"
        HS_LINE_AMOUNT_FIELD_KEY = "hs_line_amount"
        ...
        RULE = { number: 6, key: "hs_rebate_math_within_cap" }.freeze
        ...
        def rebate_cap_evaluation
          ...
          remediation_amount = decimal_from_field(line_amount_field)
          income_level = parsed_income_level
          cap_config = income_level ? CAPS_BY_INCOME_LEVEL[income_level] : nil
          calculated_cap = calculated_cap(remediation_amount, cap_config)
          failures = []
          warnings = []
          ...
          if remediation_amount.nil?
            warnings << "#{HS_LINE_AMOUNT_FIELD_KEY} is missing or not numeric"
          end
          if income_level.nil?
          ...
          if income_level == 3 && rebate_amount&.positive?
            failures << "income level 3 has no health and safety remediation rebate"
          end
          ...
          if rebate_amount && remediation_amount &&
               rebate_amount > remediation_amount
            failures << "rebate #{money(rebate_amount)} exceeds health and safety remediation amount #{money(remediation_amount)}"
          end
        ...
        def calculated_cap(remediation_amount, cap_config)
          return nil unless remediation_amount && cap_config

          percentage_cap = remediation_amount * cap_config.fetch(:percent)
        ...
        def line_amount_field
          @line_amount_field ||= best_genai_field(HS_LINE_AMOUNT_FIELD_KEY)
        end
        ...
        def rebate_cap_reason_text(result:, failures:, warnings:)
          case result
          when "pass"
            "The named rebate, health and safety remediation amount, and income-level cap are all present, and the rebate is no greater than the eligible remediation amount or calculated cap."
          when "fail"
            "The deterministic health and safety remediation rebate comparison failed: #{failures.join("; ")}."
          else
            "Code could not confidently complete the health and safety remediation rebate comparison because #{warnings.join("; ")}. Admin should verify the invoice rebate line, remediation line amount, and matched eligibility code."
          end
        end

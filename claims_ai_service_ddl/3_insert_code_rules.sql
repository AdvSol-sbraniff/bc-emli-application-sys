...
WITH code_rules_seed (
  ...
  (
    ...
    'Requirement PDF: ASHP requirements tables for electric, wood, gas/propane, and oil conversion sections. Uses the matched AHRI/OHPA product row plus hp_efficiency_and_capacity and hp_make_model evidence for variable-speed wording.',
  ...
  (
    ...
    'Checks electrical service upgrade timing against the associated heat pump or heat pump water heater installation date. Pseudocode: 1. Use invoice_versions.di_ocr_invoice_date as the ESU installation-date proxy. 2. Read the GenAI-located heat pump installation date from esu_heat_pump_installation_date_reference. 3. When both dates are parseable, pass when the invoice-date proxy is within six months before or after the heat pump installation date; fail when it is outside that window. 4. Warn when either required date is missing or not parseable. 5. Supporting-document dates do not override the invoice-date proxy in this deterministic rule; semantic evidence reconciliation belongs in GenAI. 6. In calculation, show both dates, their sources, the allowed window, and whether the invoice-date proxy is within it.',
    true,
    ...
    'Review the invoice date used as the ESU installation-date proxy and the associated heat pump installation date before deciding whether the six-month timing requirement was met.',
    ...
    NULL,
    'Requirement PDF: ELECTRICAL SERVICE UPGRADE rebate requirement 3. Uses invoice_versions.di_ocr_invoice_date as the ESU installation-date proxy and esu_heat_pump_installation_date_reference as the GenAI-located associated heat pump installation date. Supporting-document dates are intentionally not reconciled by this deterministic rule.',

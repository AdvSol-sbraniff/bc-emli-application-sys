...
WITH field_seed (
  ...
  ('utility_bill_or_account_document', 'strata_or_landlord_account_evidence', 6, 'Locate any text indicating the utility account is held by a strata corporation, landlord, property manager, or other non-resident/non-homeowner account holder.', true),
  ('utility_bill_or_account_document', 'utility_service_type_or_fuel_evidence', 7, 'Locate utility service type or fuel evidence, such as electric service, natural gas, propane, oil delivery, or other account/service wording.', true),
  ('utility_bill_or_account_document', 'account_number_or_reference', 8, 'Locate the account number, customer number, meter number, premise ID, service ID, or similar utility account reference if visible.', true),
  ('utility_bill_or_account_document', 'fuel_consumption_quantity_or_period', 9, 'Locate fuel or utility consumption quantities and service periods if visible, such as litres of oil delivered, kWh, GJ, cubic metres, billing period, annual consumption, or delivery period.', true),
  ...
  ('fossil_removal_proof', 'removal_scope_or_description', 5, 'Locate the description of fossil-fuel removal/decommissioning work completed, including appliance, piping, vent, fuel container, tank, capping, disconnection, or by-law/compliance wording.', true),

  ('fossil_modification_or_removal_proof', 'modified_or_removed_equipment_type', 1, 'Locate the fossil-fuel equipment type modified, removed, capped, decommissioned, disconnected, or retained as part of the dual-fuel upgrade.', true),
  ('fossil_modification_or_removal_proof', 'modification_or_removal_date_or_permit_reference', 2, 'Locate the modification/removal/decommissioning date, inspection date, permit number, or permit date.', true),
  ('fossil_modification_or_removal_proof', 'site_address', 3, 'Locate the site address for the fossil-fuel modification or removal work.', true),
  ('fossil_modification_or_removal_proof', 'contractor_or_authority_name', 4, 'Locate the contractor, fuel supplier, inspection authority, permit authority, or heat-pump installation company name if visible.', true),
  ('fossil_modification_or_removal_proof', 'modification_or_removal_scope_or_description', 5, 'Locate the description of work completed pertaining to fossil-fuel system modification or removal, including controls, capping, piping, appliance, vent, tank, or by-law/compliance wording.', true),

  ('utility_bill_or_invoice', 'utility_provider', 1, 'Locate the utility provider name.', true),
  ('utility_bill_or_invoice', 'previous_service_size', 2, 'Locate previous electrical service size if visible, such as 60 amp or 100 amp.', true),
  ('utility_bill_or_invoice', 'new_service_size', 3, 'Locate new/upgraded electrical service size if visible, such as 100 amp, 200 amp, or 400 amp.', true),
  ('utility_bill_or_invoice', 'service_address', 4, 'Locate the service address for the utility service upgrade.', true),
  ('utility_bill_or_invoice', 'service_completion_or_invoice_date', 5, 'Locate utility invoice date, completion date, connection date, or service upgrade date.', true),
  ('utility_bill_or_invoice', 'utility_upgrade_cost_or_reference', 6, 'Locate utility upgrade cost, connection fee, service upgrade reference, work order, invoice number, or account reference.', true),
  ...
  ('energy_performance_label', 'product_category_or_system_type', 6, 'Locate the product category or system type shown on the label, such as window, door, HRV, ERV, bathroom fan, heat pump, or water heater.', true),
  ('energy_performance_label', 'label_legibility_concern', 7, 'Locate or summarize any visible blur, cutoff, glare, low resolution, or other energy-label legibility issue.', true),
  ...
  ('product_spec_sheet', 'continuous_duty_motor_evidence', 11, 'Locate continuous-duty, permanently lubricated motor, or similar bathroom-fan motor evidence if visible.', true),
  ('product_spec_sheet', 'backdraft_damper_evidence', 12, 'Locate self-closing backdraft damper evidence if visible.', true),
  ...
  ('approved_heat_load_calculation', 'calculation_standard_reference', 5, 'Locate CSA-F280-12, F280, heat-load standard, software, or methodology reference if visible.', true),
  ('approved_heat_load_calculation', 'approval_status_or_condition', 6, 'Locate approval status, approval condition, limitation, revision requirement, or program acceptance wording if visible.', true),
  ...
  ('commissioning_or_control_document', 'backup_fuel_or_integration_evidence', 4, 'Locate fossil-backup fuel, propane/natural-gas integration, dual-fuel wiring/control integration, or retained backup-system evidence if visible.', true),
  ('commissioning_or_control_document', 'region_or_temperature_threshold_evidence', 5, 'Locate region, climate zone, outdoor-temperature threshold, or region-specific switchover threshold evidence if visible.', true),
  ...
  ('non_integrated_area_preapproval_notice', 'preapproval_date', 1, 'Locate the Non-Integrated Area preapproval, approval, email, notice, or decision date if visible.', true),
  ('non_integrated_area_preapproval_notice', 'approval_reference', 2, 'Locate approval number, application/reference number, email subject/reference, case number, or program reference if visible.', true),
  ('non_integrated_area_preapproval_notice', 'non_integrated_area_evidence', 3, 'Locate Non-Integrated Area, electrical-grid area, remote-grid, BC Hydro/FortisBC limitation, or other non-integrated-area wording.', true),
  ('non_integrated_area_preapproval_notice', 'approved_upgrade_scope', 4, 'Locate the heat pump, water heater, hydronic, dual-fuel, or other upgrade scope covered by the preapproval notice.', true),
  ('non_integrated_area_preapproval_notice', 'property_or_participant_reference', 5, 'Locate property address, participant/customer name, eligibility code, or other case reference visible on the notice.', true),

...
WITH field_seed (
  ...
  ('utility_account_document', 'utility_provider', 1, 'Locate the utility provider name, such as BC Hydro, FortisBC, New Westminster, Penticton, Nelson Hydro, Summerland, Grand Forks, or Pacific Northern Gas.', true),
  ('utility_account_document', 'account_holder_name', 2, 'Locate the utility account holder/customer name if visible.', true),
  ('utility_account_document', 'service_address', 3, 'Locate the utility service address or premises address if visible.', true),
  ('utility_account_document', 'account_or_bill_date', 4, 'Locate the account document date, statement date, account date, or service-period date if visible.', true),
  ('utility_account_document', 'residential_account_evidence', 5, 'Locate text indicating this is a residential account, residential service, domestic service, or similar non-strata/non-landlord account evidence.', true),
  ('utility_account_document', 'strata_or_landlord_account_evidence', 6, 'Locate any text indicating the utility account is held by a strata corporation, landlord, property manager, or other non-resident/non-homeowner account holder.', true),
  ('utility_account_document', 'utility_service_type_or_fuel_evidence', 7, 'Locate utility service type or fuel evidence, such as electric service, natural gas, propane, oil delivery, or other account/service wording.', true),
  ('utility_account_document', 'account_number_or_reference', 8, 'Locate the account number, customer number, meter number, premise ID, service ID, or similar utility account reference if visible.', true),
  ('utility_account_document', 'fuel_consumption_quantity_or_period', 9, 'Locate fuel or utility consumption quantities and service periods if visible, such as litres of oil delivered, kWh, GJ, cubic metres, billing period, annual consumption, or delivery period.', true),

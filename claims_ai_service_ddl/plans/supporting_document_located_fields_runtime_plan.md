...
Add a supporting-document extraction slice that keeps final ESP pass/warn/fail decisions in the common/upgrade GenAI rulechecks, while letting the support-document triage call extract reusable facts from each uploaded supporting document.

## Runtime Shape

1. Every uploaded file still receives DI read OCR first.
2. The triage/classifier call still decides `document_kind` and `supplement_type_key`.
3. `claims.validationgenai_config.supporting_document_extraction_mode` selects one of two paths:
   - `combined_with_classifier`: the classifier also receives located-field tasks and returns `supporting_document_located_fields`.
   - `separate_extraction`: the classifier routes only; the bundle then runs a `supporting_document_extraction` step for typed supporting documents with configured located fields.
...
- `claims.validationgenai_config`
  - Stores `classifier_combined_with_extraction_system_record`, `classifier_without_extraction_system_record`, `supporting_document_extraction_system_record`, and `supporting_document_extraction_mode`.
...
- Unit/smoke test persistence with synthetic classifier payloads for multiple support document types.
...
- Implemented the classifier located-field prompt extension and runtime persistence path.
- Added split-mode config for combined classifier extraction vs separate supporting-document extraction.
- Added `supporting_document_extraction` ingest step orchestration for the separate mode.
...
- Ran real PDF batches from `claims_ai_service_documentation/pdf test files`:
  ...
  - Split-mode real PDF run `3f5ee3ca-8307-4498-8dcf-1333b562d262` completed with two `supporting_document_extraction` steps, invoice `2a0a5f02-969e-4588-9f27-13257f89e39d` reached `genai_complete`, and 9 supporting-document located fields were persisted.
